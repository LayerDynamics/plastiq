...
| **2** | **Clean-room a deterministic B-rep traversal layer** (half-edge `next/mate/face/edge` arrays + topological-walk neighborhoods + 5° dihedral convexity) from BRepNet's *featurizer idea* | T2 + T1 | **clean-room only** (BRepNet = CC-BY-NC-SA) | **Med** | **Yes** | T2: feature-recognition to steer segment-then-fit (group tangent faces, flag fillets/holes). T1: "select tangent faces / fillet chain / convex edges" — a real authoring-UX gap (we stop at `faceByNormal`). |
| **3** | **Evaluate** DLR-RM `shape-completion` as a separate optional GPU service for **"Complete Scan / Fill Gaps"** (partial mesh/scan → watertight mesh → reconstruct) | T2 (own service) | **MIT** | **High** (no checkpoints → train on ShapeNet; class-dependent) | **Yes** | Only modern+MIT+maintained completion path; genuinely new capability (completes the *user's* input). Gate as "spike + evaluate," not build-now. |
| **4** | **Document a photogrammetry capture workflow** (nerfstudio, or sdfstudio for watertight output) as an **external tool → export PLY → import → reconstruct** | T2 stretch / external | **Apache-2.0** | **Low** (docs) / High (bundle) | **Yes** | Real-object photos→mesh, which the cloud path doesn't do. Honest caveat: organic output, weak for mechanical parts, GPU-only. Document first; don't bundle. |
