...
| **R6.5** | **✅ shipped — builder + bounded topology integration** — `app/freeform.py`: `freeform_face` (boundary loop as C0 edge constraints + interior point constraints → `BRepOffsetAPI_MakeFilling`) and `freeform_region_face` (a connected mesh region's outer loop + subsampled interior) build a smooth face approximating a curved patch. **Topology integration (`freeform_capped_solid`):** a freeform cap whose boundary IS the same straight mesh polyline its planar neighbours use **does join a watertight solid** — the coincident boundaries sew at 1e-6 (`NbFreeEdges()==0` → `MakeSolid` → `OrientClosedSolid`), volume-validated (a box + a bulged freeform top → a 6-face solid; `test_freeform.py`). **Still future (honest):** the auto pipeline does NOT yet segment freeform-vs-planar regions in an arbitrary organic mesh and hand their shared boundaries to this builder; and the **analytic-rim sagitta case** (a smooth fitted arc replacing a faceted polyline neighbour, deviating ≫ the sew gate — §D-3) still needs the surface-intersection tail. Arbitrary closed organic blobs keep the (valid) faceted solid; nothing is dropped. | ✅ shipped (bounded) |
| **R6.7** | **✅ shipped — integration + browser E2E.** (a) `apps/plastiq/src/ai/reconstruct.integration.test.ts` (keyed `RECONSTRUCT_URL`): real client → HTTP → live service → STEP (cylinder→`cylinder`, stepped shaft→`revolution`). (b) `e2e/plastiq/reconstruct.spec.ts`: full browser E2E — open a mesh doc → "Convert to CAD" → service → STEP → kernel `importStep` (real OCCT) → rendered B-rep part (faceCount>0), verified live; skips when the service is unreachable. (c) Service CORS added (`main.py`) so the browser can call it cross-origin. Server covered by 47 live-OCCT pytest. | ✅ shipped |
