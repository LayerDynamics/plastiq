...
| **R6.4b-iii** | **✅ shipped (bounded — CSG booleans)** — non-coaxial mixed parts via the InverseCSG paradigm (MIT): build primitive solids, combine with OCCT booleans (`BRepAlgoAPI_Cut`) so the boolean engine computes the shared-edge topology — no fragile manual surface intersection. `app/csg.py`: axis-aligned box (AABB) − cylindrical through-holes (hole vs boss decided by wall-normal direction); volume-self-validated; wired into `auto` (primitive → revolution → CSG → fitted). A box-with-hole GLB → a 7-face boolean solid (`test_csg.py`). General arbitrary non-coaxial parts (bosses, non-axis-aligned bases, multi-primitive trees) remain future; faceted fallback keeps them valid. | ✅ shipped (bounded) |
