...
|---|---|---|---|---|---|---|
| **1** | ✅ **SHIPPED** — Scaled-Chamfer-Distance fidelity metric (from StepForge's `step_to_pointcloud`+`scd_reward`; `alignment` stage skipped — same frame) in `services/reconstruct` (`app/fidelity.py`; `surface_deviation`/`fidelity_tol` on the report, surfaced in the convert UI; advisory not a gate — see `docs/adr/0001`) | T2 | **Apache-2.0** | **Low** (**MLX** metric math — sampling + brute-force Chamfer; no open3d) | **Yes** | *Surface*-level fidelity score; complements the volume-only + per-region-RMS gates. Lets reconstruct honestly report faithfulness. 12 pytest + 1 vitest. |
...
| **3** | ✅ **BUILT — deployable service, no browser client yet** (M8) — **MLX shape completion** (`services/capture/` `/complete`): a conditional occupancy network (PointNet enc + occupancy dec) completes a partial scan → full watertight mesh → existing MeshDoc→reconstruct. Trains on the **M4 Max** (DLR-RM is CUDA-only → own MLX impl). | T2 | **MIT** | High | **Yes** | Completes the *user's* partial input (fills holes the scan never saw). Demo trains on synthetic spheres; general objects need ShapeNet training + a checkpoint (ADR 0008). 4 pytest incl. real MLX training that fills a missing hemisphere. |
| **4** | ✅ **BUILT — deployable service, no browser client yet** (M7) — **MLX neural-SDF capture service** (`services/capture/`): oriented point cloud → IGR Softplus SDF (MLX, trains on the **M4 Max**) → marching-cubes mesh → GLB → existing MeshDoc→reconstruct. FastAPI submit→poll. Not a CUDA nerfstudio port (won't run on Apple Silicon) — a self-contained MLX impl. | T2 | **Apache-2.0** | High | **Yes** | Real surface reconstruction from points/depth. SfM (photos→points) stays COLMAP's job (ADR 0007). 13 pytest (real MLX training on a sphere, ~6 s). SPEC-10 §capture. |
| **5** | ✅ **BUILT+TESTED** (M6) — kornia `depth_to_normals`/`unproject_depth` + a pinhole camera, ported to **MLX** (Apple Silicon) in `services/capture/app/geometry.py`. ⚠️ **standalone util — not yet called by `/capture`** (which takes oriented points as input; geometry.py is the depth-scan front-end, wired when a depth-ingestion endpoint is added). Nister-5pt/Kannala-Brandt deferred (ADR 0006). | T2 | **Apache-2.0** | Low | partial→yes | The depth/point-cloud math a depth-scan path needs. 6 pytest. |

### 2a. Integration status — wired vs built-but-not-wired (read before "shipped")

All 10 milestones are **built, strict-TDD'd, and committed** (reconstruct 85 pytest · capture 16 pytest +1 gated · vitest 1095 · typecheck + lint clean). But "shipped" ≠ "reachable from the app a user runs". Honest state:

| Milestone | Status | Reachable from the running app? |
|---|---|---|
| **M1** SCD fidelity | ✅ **LIVE** | Yes — `pipeline → report → GenerationPanel` convert UI |
| **M2** BRepNet selectors + recognition | ✅ **LIVE** | Yes — worker `rebuild.ts` resolves the new selectors; agent prompt documents them; recognition in the report |
| **M5** agent planning-IR | ✅ **LIVE** | Yes — `plan_part` in `buildAgentTools` + the agent prompt |
| **M4** `.assy` assembly + BOM | 🟡 **tested island** | No — `assy.ts`/`BomPanel.tsx` imported only by their tests; not mounted in the app UI |
...
| **N0–N12** MLX NeRF/VolSDF service (`services/nerf`) + `@plastiq/nerf` | ✅ **LIVE — wired** | Yes — the **first** self-hosted ML service with a wired browser client: FastAPI `/train` (posed images → VolSDF/NeRF → GLB, trains on the M4 Max) ← the `@plastiq/nerf` workspace package (`trainNerf`) ← `apps/plastiq/src/ai/nerf.ts` `captureFromPhotos` ← the **"Capture from photos"** panel → `MeshDoc` (`photos3d`) → existing Convert-to-CAD. 43 service pytest (incl. real submit→poll→GLB API, both models) + 6 package vitest + 4 app vitest. SPEC-11 / ADR 0011. |
...
- **partcad** — ✅ **BUILT+TESTED (M4), not yet mounted in the app UI**: declarative `.assy` description + `realizeAssembly` + **auto-BOM** (`apps/plastiq/src/assembly/assy.ts` + `BomPanel.tsx`; own dependency-free JSON schema, not partcad's YAML/Python). `assy.ts`/`BomPanel` are imported only by their tests — the panel isn't rendered in the shell yet. SPEC-9 §assembly + ADR 0004; 11 tests. (Git-versioned packages + multi-part geometry library remain future work.)
