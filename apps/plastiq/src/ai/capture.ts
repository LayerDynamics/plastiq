...
// available. The dependency direction is app → @plastiq/capture (never the reverse). The service
// exposes no auth (main.py), so unlike nerf.ts there is no API-key threading here.
...
export async function meshFromPointCloud(
  ...
  const result = await capturePointCloud(input, opts);
...
export async function meshFromPartialScan(
  ...
  const result = await completePartialScan(input, opts);
