...
same shape services/capture and services/reconstruct already expose, so the browser reuses one polling
client. Jobs run as background tasks on the event loop; the heavy MLX training is dispatched to a
worker thread by the caller (so the loop stays responsive). Identical contract to capture `jobs.py`.
"""
