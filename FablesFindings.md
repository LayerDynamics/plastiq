...
| Gate | Command | Result |
| --- | --- | --- |
| Kernel tests | `npx vitest run packages/cad` | **88 files, 359 tests — all green** (on the REBUILT wasm) |
| App tests | `pnpm --filter plastiq test` | **306 files pass, 0 fail** (1973 passed, 6 skipped) |
...
4. **What you see is not what you save, export, or simulate.** The production build doesn't compile; placements compose into nothing downstream (parts teleport to origin at sim start; exports ship unposed single bodies with the assembly silently dropped); AI/service applies wipe the undo history and commit unvalidated STEP; mesh edits bypass autosave and the unsaved-changes guard (§2.3, §2.11, §2.12).
...
### 2.2 Booleans never unify coplanar faces — post-join selection silently operates on fragments

**Where:** `packages/cad/src/action/boolean.ts:12-30` returns the raw `BRepAlgoAPI` result. **Zero** uses of `ShapeUpgrade_UnifySameDomain`, `SetFuzzyValue`, or `SetNonDestructive` anywhere in `packages/cad/src` (verified by grep).
...
3. **Deps are decorative — deletion/suppression/reorder silently rebinds geometry.** `removeFeature` never touches other features' `deps`/`data.sketchId` (`store.ts:425-435`); nothing validates deps; when the named sketch is missing, `sketchForFeature` **falls back to the most recent other sketch** (`rebuild.ts:179-193`). Delete or suppress Sketch A → its extrude silently rebuilds from Sketch B's profile — wrong geometry, zero error. Same on reorder above the dep (`store.ts:443-456`, `FeatureTree.tsx:149,161` have no dependency check); FeatureTree's Delete key removes with no confirm and no dependent check (`FeatureTree.tsx:317-319`). Most ribbon-created features carry **no deps at all** (`registry.ts:281,329,349-364,379-388,404-446,487-492`), so most documents live on this fragile fallback.
4. **No parameter validation between UI/AI and kernel.** `NumberField` commits any finite number (`PropertiesPanel.tsx:29-33`); `updateParams` merges blindly (`store.ts:397-403`); zod uses bare `z.number()`; nothing caps `count` — `linearPattern count: 1e6` runs on the app's **single** geometry worker (AI probe path `ai/agentTurn.ts:107-116`) and freezes all interactive rebuilds without ever erroring.
...
3. **`.assy` round-trip destroys mates and joints** — schema has no mates/joints vocabulary (`assembly/assy.ts:36-44,211-222`); import also grounds nothing (`assy.ts:172-190`) so an imported assembly **free-falls entirely** at Simulate (contrast `addInstance` grounding instance 0, `store.ts:548`); `.assy` `part` names bind no geometry — N instances all clone the currently open part (`assy.ts:6-7`, `lower.ts:45-72`), with no caveat in the import status.
4. **Editing during a running sim never invalidates it.** Only context-menu actions gate on `!ctx.simulating` (`config.ts:137`); PropertiesPanel/FeatureTree/AssemblyTree have zero gates (grep verified by agent); rebuilds keep updating the rendered mesh mid-sim while the manifest/colliders stay stale; only `simExperiment` changes restart (`store.ts:709-714`).
5. **Joint frames go stale:** `applyJoint` bakes world-frame origin/axis at creation (`store.ts:662-687`); `solveAssembly` re-poses instances but never re-anchors joints, and the mate solver ignores joints (`assembly/model.ts:205-229`) — lowered constraints attach at outdated frames (`lower.ts:78-91`). Experiment ground height uses COM + **unrotated** hull extents (`sim/experiments.ts:100-116`) — wrong for rotated instances.
...
1. **Every AI/ML apply lands through `loadDocument`, which wipes undo history** (`store.ts:806-816`, verified: `past: [], future: []`) — `ai/tools/buildPart.ts:25`, `ai/agentTurn.ts:81,116`, `three/contextmenu/mlActions.ts:87` (which also sets `currentId: null`, detaching the open project), `ai/nurbs.ts:35`. One accepted AI edit makes an hour of manual edits un-undoable.
...
| Join two pads → shell/fillet the top | acts on a coplanar fragment | §2.2 |
...
| Move a part → Simulate | part teleports to origin (placement never lowered) | §2.11.1 |
| Export a mated assembly to STEP | one unposed body; assembly dropped, no warning | §2.11.2 |
| Import `.assy` → Simulate | nothing grounded → whole assembly free-falls | §2.11.3 |
...
- **P1** Box is the only primitive (`solid/primitives.ts`); no cylinder/sphere/cone/torus — the broken sketcher is a single point of failure for round geometry. **P1** No scale operation (`action/transform.ts` = translate/rotate/mirror).
...
| W3 | P1 | Stale e2e spec clicks now-disabled demo Loft/Sweep buttons → Playwright timeout | `e2e/plastiq/cad-features.spec.ts:271-313`; `store/seed.ts:8-13`; `ribbon/ActionButton.tsx:27,39,56` |
...
| Placement → sim manifest | **BROKEN** (§2.11.1) | `geometry.worker.core.ts:52-69`; `lower.ts:59-72`; false comment `rebuild.ts:801-805` |
| Placement → STEP/IGES/glTF | **BROKEN** (§2.11.1) | `geometry.worker.core.ts:103-120` |
...
| Joints UI → sim constraints | PARTIAL — stale world frames (§2.11.5) | `store.ts:662-687`; `lower.ts:78-91` |
| `.assy` import → live assembly | Wired, but no ground + clone-parts caveat (§2.11.3) | `registry.ts:157-209`; `assy.ts:118-190` |
| `.assy` export | LOSSY — mates/joints dropped (§2.11.3) | `assy.ts:211-222` |
...
| # | Sev | Finding | Evidence |
| --- | --- | --- | --- |
| I1 | P1 | **STEP unit handling absent → 1000× interop error.** Kernel is SI metres; neither export nor import configures OCCT units (no `Interface_Static` anywhere); OCCT defaults to MM → exported 40 mm part declared 0.04 mm; real-world mm STEP imports 1000× large. Hits AI-reconstruct too (`ai/reconstruct.ts:61-64`). Masked because `io.test.ts:17` round-trips its own export. Fix needs `Interface_Static` in `occt.build.yml` (wasm rebuild). | `io/index.ts:20-62`; `unit/index.ts:1-2` |
