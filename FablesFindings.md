...
- **SECOND WASM REBUILD (2026-07-18)** — `just cad-occt` run again after adding four symbols that were present in the `.d.ts` only as parameter/return TYPES (so they looked available but could not be constructed): **`gp_Quaternion`** (+ its ctor variants), **`TopoDS_Compound`**, **`BRep_Builder`** and its base **`TopoDS_Builder`**. Verified on the new binary (`oc/bindings.test.ts`): `new gp_Quaternion_2(x,y,z,w)` drives `gp_Trsf.SetRotation_2` — so `transformRigid()` now hands the product's end-to-end quaternion pose straight to OCCT instead of hand-building a rotation matrix for `SetValues` — and `BRep_Builder.MakeCompound`/`Add` assembles a REAL two-body compound (volume sums; it explores as 2 unwelded solids). This does **not** change assembly EXPORT, which keeps using repeated `Transfer`/`AddShape` to preserve per-body identity (fusing would weld mated parts). The same run also disproved the `Standard_Failure` prediction — see §2.5.4.
...
4. **What you see is not what you save, export, or simulate** — ✅ **the §2.11 chain is now closed** (2026-07-17/18): the production build compiles (§2.3), placements compose into the sim manifest AND file exports (§2.11.1), assemblies export as N posed bodies (§2.11.2), `.assy` round-trips its mates/joints/ground (§2.11.3), editing during a run auto-stops the sim (§2.11.4), joint frames re-anchor on solve (§2.11.5), and AI/ML applies preserve undo history (§2.12.1). **§2.12 is closed too:** AI/ML applies preserve undo (§2.12.1), service STEP is validated before it can replace anything (§2.12.2), mesh edits autosave/snapshot/prompt (§2.12.3), and persistence timers no longer cross projects (§2.12.4).
...
4. **`Standard_Failure` → message — PARTIALLY fixed; full decode is WASM-GATED.** New `packages/cad/src/oc/error.ts` (`describeOcctError`) replaces every `(err as Error).message` / `String(err)` on the build path, so `rebuild failed: undefined` and `rebuild failed: 5286968` are gone, and `featureErrorMessage()` guarantees every error names its feature even for a raw OCCT throw. **Reading OCCT's OWN message text is not possible in this build:** it needs either the `Standard_Failure` class bound (absent from `occt.build.yml`) or Emscripten's `getExceptionMessage` (absent — it needs `-sEXPORT_EXCEPTION_HANDLING_HELPERS`, and `occt.build.yml:143-151` documents that overriding `emccFlags` REPLACES the builder's known-good defaults and yields an unlinkable wasm). **CORRECTION (2026-07-18) — that prediction was TESTED AND IS WRONG.** `Standard_Failure` was added to `occt.build.yml` and `just cad-occt` WAS run: the class is now bound and constructible, and message decoding still does **not** work. Measured against the new binary: an OCCT throw arrives as a raw pointer (a plain `number`), `instanceof oc.Standard_Failure` is `false`, the value carries no `GetMessageString`, and the module exports no `___cxa_begin_catch` to adjust the pointer with. Binding a class makes it CONSTRUCTIBLE; it does not change how a C++ throw crosses into JS. The only remaining route is `-sEXPORT_EXCEPTION_HANDLING_HELPERS`, i.e. an **emccFlags/build-system change, not a symbol addition**. Pinned by `oc/bindings.test.ts` ("Standard_Failure binds but an OCCT throw still arrives as a raw pointer") so it is not attempted a third time; `describeOcctError` keeps degrading honestly ("the geometry kernel rejected this operation…") rather than leaking a pointer.
...
1. **`placement` composes into nothing downstream** — ✅ **FIXED (2026-07-17).** One shared conversion (`viewport/placement.ts`, now runtime-THREE-free so the geometry worker imports it; pure Euler↔quat proven against THREE ground truth incl. gimbal lock): `placementPoseOf` poses the synthesized `body0` in `effectiveAssembly` AND seeds the viewport's `simBodies`, so Simulate starts the part exactly where the viewport shows it — and the drop-test ground/lift now compute from the posed manifest body. File export bakes the placement into the exported geometry so STEP/IGES/glTF carry the posed part — via `exportPoses`, which reads those same `effectiveAssembly` poses (the §2.11.2 fix collapsed export's Euler-chain into that one shared path). The once-false `rebuild.ts` / `placement.ts` comments now state the true behavior. Live-OCCT tests (`geometry.worker.core.test.ts`): a placed box lowers with manifest COM = Rz(π/2)·localCom + 0.5 m, and its exported STEP round-trips through `importStep` with the composed COM. Was: The rebuild skips it *claiming* "composed into the sim manifest at export" (`rebuild.ts:801-805`) and `viewport/placement.ts:5-6` claims M4 composition — **both comments are false**. The only consumers are the viewport (`Scene.tsx:185-195`) and the edit UI (`PropertiesPanel.tsx:60`). `effectiveAssembly` synthesizes an **identity-posed** `body0` (`worker/geometry.worker.core.ts:52-69`, verified); `lowerAssembly` reads only instance poses (`worker/lower.ts:59-72`). Gizmo-drag a part to (0,0,0.5) → press Simulate → the part **teleports to origin** (`Viewport.tsx:54-62` renders manifest poses); drop-test ground/lift are computed from the un-posed body.
2. **File export ships ONE unposed body; the assembly is silently dropped** — ✅ **FIXED (2026-07-18).** Export now reads the SAME `effectiveAssembly` poses Simulate does (`exportPoses` in `geometry.worker.core.ts`), so the file carries every body the viewport shows: N posed assembly instances, or the single placement-posed part. STEP/IGES pose real geometry per body — new kernel `transformRigid` applies a quaternion+translation in one `gp_Trsf.SetValues` call — and ship them through `exportStepAssembly`/`exportIgesAssembly`, which accumulate N shapes into one file via repeated `Transfer`/`AddShape`. **Fusing was rejected deliberately:** a boolean would WELD mated parts into a single solid, destroying per-body identity. (At the time a hand-built compound was not available either — `BRep_Builder`/`TopoDS_Builder` were unbound. Both are now bound and verified by the 2026-07-18 rebuild, and the export path still uses repeated `Transfer`/`AddShape` on purpose: a compound is the right in-memory representation, not a substitute for per-body transfer.) glTF gained real **node transforms**: one shared mesh instanced by N nodes carrying TRS, so geometry is transmitted once regardless of instance count (`exportGltfAssembly`). The status line reports the body count (`exported STEP — 3 bodies`), so "exported STEP" can never again mean "exported one body and dropped the rest". Live-OCCT tests: a 3-instance assembly's STEP re-imports at exactly 3× the single-body volume with the composed centroid; its glTF has 1 mesh / 3 nodes at the expected TRS. **Still true from the original finding:** glTF carries positions+indices only — no normals or materials. Was: The worker `export` op rebuilds features only (`geometry.worker.core.ts:103-120`, verified) — `req.doc.assembly` never consulted. A 3-instance mated assembly exports as one copy at the local frame; status reports "exported STEP" unconditionally (`registry.ts:96-119`). glTF is positions+indices only — no normals, materials, or node transforms (`packages/cad/src/io/index.ts:100-170`).
...
**Status (2026-07-18): all five items above are FIXED** — placement composes into Simulate and every export (§2.11.1), assemblies export as N posed bodies with a body-count status (§2.11.2), `.assy` carries mates/joints/ground (§2.11.3), document edits auto-stop a running sim with a note (§2.11.4), and joint frames re-anchor on solve with orientation-correct ground heights (§2.11.5). What deliberately remains, documented above rather than silently folded in: glTF still carries positions+indices only (no normals/materials); `.assy` `part` names still bind no geometry (the multi-part library milestone — now disclosed in the import status); and the mate solver still does not treat joints as constraints.
...
2. **Service STEP is committed destructively before validation** — ✅ **FIXED (2026-07-18).** New `commitStepDocument(doc, {probe, load})` in `ai/reconstruct.ts` PROVES the document builds before it replaces anything, and throws (without calling `load`) when it doesn't — so the store and the open mesh both survive a bad service response, and the caller's existing error path reports why. The probe is the SAME real-OCCT one the AI's `build_part` tool uses (`geometryClientProbe`, which rejects a *partially*-failed document, not just a null mesh), now exposed as `liveBuildProbe()` next to `buildSeam` in `agentTurn.ts`. **All five landing sites** go through the gate: `mlActions` reconstruct + nurbs, `GenerationPanel` reconstruct + nurbs, and the agent's own `reconstruct_brep`/`fit_nurbs` tools (`tools/meshToCad.ts` — which had the identical unvalidated-commit bug, found while wiring this). `FitMeshToCadDeps.probe`/`MeshToCadDeps.probe` are REQUIRED, so the type system now rejects any new call site that skips validation. When the geometry worker isn't ready, the probe refuses rather than waving the document through. Tests: unbuildable STEP is never committed and the mesh is kept (`mlActions.test.ts`, `nurbs.unit.test.ts`, `meshToCad.unit.test.ts`), and the probed document is byte-identical to the committed one. Was: `runReconstructBrep`/`fitMeshToCad` call `loadDocument` with the returned STEP un-parsed (`mlActions.ts:51`, `ai/nurbs.ts:67-70`); garbage STEP fails later in the worker (`rebuild.ts:777-799`) → empty viewport, no undo, source mesh discarded (`mlActions.ts:88-89`).
3. **Mesh-document edits bypass autosave, crash recovery, AND the unsaved-changes guard** — ✅ **FIXED (2026-07-18).** For a generated/sculpted mesh project the MESH *is* the document, and it lives in the projects store's `activeMeshDoc` rather than the cad or voxel store, so both watchers needed their own subscription to it: `wireAutosave` now schedules the same debounced recovery snapshot + autosave on a mesh edit, and `unsavedGuard` marks the session dirty. The recovery machinery already supported mesh docs (`liveDocument`/`toRecoveryDoc` wrap a MeshDoc), so nothing else changed. Both guards skip non-edits identically: a null doc is LEAVING mesh mode, `busy` covers `recover()`, and `prev.busy` covers `open()` — which clears `busy` in the SAME atomic set that installs the opened doc, so checking only `s.busy` would read false and treat a load's tail as an edit. Tests: new `projectsStore.meshAutosave.test.ts` (mirroring the voxel one) drives the REAL init/wireAutosave/writeRecovery machinery — a sculpt edit lands a dirty envelope at 500 ms and autosaves at 1500 ms, and the full crash round-trip (edit → crash → init → recover) restores the edited mesh; `unsavedGuard.test.ts` covers the prompt. Was: Mesh edits write `activeMeshDoc` (`Viewport.tsx:153-167`); `wireAutosave` subscribes only to cad+voxel stores (`persistence/projectsStore.ts:161-175`); `unsavedGuard.ts:37-48` never marks `"mesh edited"` dirty → close tab, all sculpt edits gone, no prompt.
4. **Stale timers cross projects** — ✅ **FIXED (2026-07-18).** The autosave timer lived in `wireAutosave`'s closure, so nothing outside could cancel it; it is now module-scope beside `recoveryTimer`, and one `cancelPendingPersistence()` clears BOTH. It runs at every point that changes what "the current project" means: `open()` (before the swap), `newProject()`, `newVoxelProject()`, and `recover()`. Cancelling is the correct semantic rather than a lossy one: those paths replace the live document wholesale, so a timer scheduled for the outgoing document could no longer read it — there is nothing left to persist by the time it would fire. Test (`projectsStore.meshAutosave.test.ts`): with both timers armed mid-edit, `newProject()` inside the debounce window means neither fires — no autosave writes the new document under the old project's id, and no snapshot records the fresh document as dirty. Was: autosave/recovery thunks read current state at fire time and are never cancelled by `open()`/`newProject()` (`projectsStore.ts:62-65,159,261-330`) — switching projects within 1.5 s drops pending edits and snapshots the *new* project as dirty.

**Status (2026-07-18): all four items are FIXED** — history-preserving `replaceDocument` (§2.12.1), validate-then-commit for service STEP with the mesh kept on failure (§2.12.2), autosave/recovery/unsaved-guard subscribed to `activeMeshDoc` (§2.12.3), and pending persistence timers cancelled on every project switch (§2.12.4).
...
| ~~Export a mated assembly to STEP~~ | ✅ FIXED — all N instances ship posed (verified: 3-instance file re-imports at 3× the volume); status reports the body count | §2.11.2 |
...
| ~~Sculpt a mesh 20 min → close tab~~ | ✅ FIXED — mesh edits autosave, snapshot for crash recovery, and arm the unsaved-changes prompt | §2.12.3 |
...
| Placement → STEP/IGES/glTF | ✅ FIXED — `exportPoses` (shared with Simulate) poses the exported geometry; STEP round-trip test | §2.11.1 |
| Assembly (instances+poses) → sim manifest | HEALTHY | `lower.ts:45-97`; `lower/export.ts:47-85`; e2e assembly-to-sim |
| Assembly → STEP/glTF export | ✅ FIXED — N posed bodies (STEP/IGES accumulate; glTF instances one mesh via node TRS); body count in the status | §2.11.2 |
...
| Service STEP → parametric doc | ✅ FIXED — `commitStepDocument` proves it builds before committing; the mesh survives a bad response | §2.12.2 |
