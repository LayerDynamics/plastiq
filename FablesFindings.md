...
**Status (2026-07-18):** items 1–3 are FIXED — history-preserving `replaceDocument` (§2.12.1), validate-then-commit for service STEP with the mesh kept on failure (§2.12.2), and autosave/recovery/unsaved-guard subscribed to `activeMeshDoc` (§2.12.3). **Still open:** item 4 — cancel pending persistence timers on project switch (§2.12.4).
